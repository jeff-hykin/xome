...
console.log(`I need to download ${JSON.stringify(repoName)} to a folder`)
...
while (true) {
    try {
        ...
        confirm("If that failed please let me ")
        prompt("Sorry I need a directory to download the project to. Press ctrl+C if you want to cancel, otherwise press enter to try again")
    }
}
...
console.log(`Next time, when you setup the project just run the following:`)
